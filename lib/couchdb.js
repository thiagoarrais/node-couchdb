...
function streamingResListener(options, cbFired, cb) {
    return function(res) {
        res.setEncoding(options.responseEncoding || 'utf8');
        res
        .addListener('data', function(chunk) {
            return cb && cb(null, chunk);
        })
        .addListener('error', function(er) {
...
exports.createClient = function(port, host, user, pass) {
  ...
  couchClient._queueRequest = function(options, cb) {
    ...
    if (options.returnType && options.returnType === 'streaming') {
        request.addListener("response", streamingResListener(options, cbFired, cb));
    } else {
...
Db.prototype.getStreamingAttachment = function(docId, attachmentId, cb) {
  return this.request({
    path: '/'+docId+'/'+attachmentId,
    responseEncoding: 'binary',
