 ...
 * bufferedResListener
 *
 * need a named listener so we can remove it in the case that we are
 * streaming the output.
 */
function bufferedResListener(options, cbFired, cb) {
    return function(res) {
        var buffer = '';
...
function streamingResListener(options, proxy) {
    return function(res) {
        res.setEncoding(options.responseEncoding || 'utf8');
        res
        .addListener('data', proxy.emit.bind(proxy, 'data'))
        .addListener('error', proxy.emit.bind(proxy, 'error'))
        .addListener('end', proxy.emit.bind(proxy, 'end'));
    }
}
...
exports.createClient = function(port, host, user, pass) {
  ...
  couchClient._queueRequest = function(options, cb) {
    ...
    if (options.data && typeof options.data != 'string' && options.data.constructor != Buffer) {
    ...
    if (options.returnType && options.returnType === 'streaming') {
        request.addListener("response", streamingResListener(options, proxy));
    } else {
        request.addListener("response", bufferedResListener(options, cbFired, cb));
    }
    request.end();
    return proxy;
  };
...
Db.prototype.saveBufferedAttachment = function(buffer, docId, options, cb) {
  options = mixin({rev: null}, options || {});
...
Db.prototype.getStreamingAttachment = function(docId, attachmentId) {
  return this.request({
    path: '/'+docId+'/'+attachmentId,
    responseEncoding: 'binary',
    returnType: 'streaming'
  });
};
